# Inflation Graph for triangle scenrio 
from os import dup
...
#Get all the nodes
nodes = list(sprial_inflation.nodes)

#Get all the observed nodes
nodes_observed = remove_element_list(sprial_inflation_hidden, nodes)
...
for node in nodes_observed:
    ancestors = list(nx.ancestors(sprial_inflation, node))
    split_index = []
    split_key = []
...
seta = ["A", "B"]
seb = ["A", "C", "C"]
# Produce True if two sets have common ancestor
not set(seta).isdisjoint(seb)
...
print(list(nx.find_cliques(sprial_injectable)))
injectable_sets = list(nx.find_cliques(sprial_injectable))

nx.draw(sprial_injectable,with_labels=True)
plt.show()

# Find all the injectable set
# For every node, find its ancestor
